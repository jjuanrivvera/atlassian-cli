...
const (
	...
	classLocal   = "local"   // touches only this machine's config; never sent to Atlassian
)
...
var localGroups = []string{
	"config", "alias", "completion", "version", "doctor", "init", "op", "help", "mcp", "agent", "update",
}
...
// in again.
var alwaysBlocked = []string{"delete", "rm", "remove"}
...
// collide across groups — `op search` reads while `issues delete` destroys, and a name-based
// rule alone would get one of them wrong. An unannotated command outside the local groups
...
func classify(cmd *cobra.Command, path []string) string {
	...
	if slices.Contains(localGroups, group) {
		// `op call` is the exception inside a local group: it reaches the live API and can
		// name any operation, including a DELETE.
		if group == "op" && leaf == "call" {
			return classDestroy
		}
		return classLocal
	}
...
//
// Rules and hooks that list only canonical paths are trivially bypassed: `atlassian issue rm`
// reaches the same code as `atlassian issues delete`. Every spelling has to be enumerated.
...
func newAgentGuardCmd(o *globalOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "guard",
		Short: "Emit safety rules that block irreversible Atlassian operations",
...
approval, and irreversible operations (delete/remove, the raw 'api' escape hatch, and
'op call', which can name any operation including a DELETE) are blocked outright.

The generated hook is the enforcement layer. Permission rules alone are literal prefixes, so
'./bin/atlassian issues delete', 'env X=1 atlassian issues delete' and quote-splitting all
...
the Bash hook is defence in depth.`),
  ...
  atlassian agent guard --host claude-code
  atlassian agent guard --host claude-code --write
  atlassian agent guard --host codex
  atlassian agent guard --host opencode --all-writes`),
