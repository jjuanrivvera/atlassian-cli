...
// NewStore selects the credential store. Precedence: an explicit ATLASSIAN_KEYRING_BACKEND;
...
func NewStore() Store {
	...
	// variable, because a non-interactive `ssh host 'atlassian …'` sources no rc to set one.
	if fromFile {
		if fs, err := NewFileStore(""); err == nil {
			return fs
		}
