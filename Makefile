# Makefile — atlassian-cli. `make verify` is the deterministic gate (CI/dev);
...
## --- OpenAPI spec pipeline (the enumeration source, GOAL.md §0 Step 1b) ---
# The raw Atlassian documents are ~4MB and are NOT checked in; the generated catalog is.
# `spec-fetch` downloads them, `spec-gen` regenerates the embedded catalog + api-manifest.json.
spec-fetch: ## download Atlassian's published OpenAPI documents into specs/
	./scripts/spec-fetch.sh
spec-gen: ## regenerate internal/catalog/catalog.json.gz + api-manifest.json from specs/
...
.PHONY: build install uninstall run dev fmt vet lint tidy test test-race \
        ...
        spec-fetch spec-gen docs-gen docs-serve docs-build snapshot setup-hooks clean
